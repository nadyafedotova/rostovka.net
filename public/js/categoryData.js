...
if (localStorage.getItem('pageFilter') && !$('h1')[0].innerText.toLowerCase().includes(localStorage.getItem('pageFilter').toLowerCase())) {
  localStorage.removeItem('filterValues');
  localStorage.removeItem('sizeValues');
  localStorage.removeItem('selectedCount');
  localStorage.removeItem('pageNum');
  localStorage.setItem('pageFilter', $('h1')[0].innerText);
}
...
function init() {
  $('.sidebar-container input[type=checkbox]').on('change', function(event) {
    ...
    let type = 'sex';
    if (targetID.includes('type')) type = 'tip';
    else if (targetID.includes('season')) type = 'season';
    ...
    if ($(this).is(':not(:checked)')) {
      if( targetID.includes('type') && $('meta[name="category_id"]')[0]) {
        $('.product--block').append('<div class="preloader"><i></i></div>');
        let type_names = [];
        ...
        else if( $('input[type="checkbox"]:checked').length == 1 && type_names.length == 1) window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }/${ type_names.join() }`;
    ...
    if ($(this).is(':checked')) {
      ...
      if( targetID.includes('type') && $('meta[name="category_id"]')[0]) {
        $('.product--block').append('<div class="preloader"><i></i></div>');
        window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }/${ $(this).data('href') }`;
      } else {
...
if ($('#filter-category .checkbox:last-child')[0].innerText.indexOf('Детское') != -1) $('#detskoe-pol').css('display', 'block');
