...
if ( localStorage.getItem('pageFilter') !== $('#titleh2')[0].innerText ) {
  localStorage.removeItem('filterValues');
  localStorage.removeItem('sizeValues');
  localStorage.removeItem('selectedCount');
  localStorage.removeItem('pageNum');
  localStorage.setItem('pageFilter', $('#titleh2')[0].innerText);
}

var data = [],
  productTheme = $('#template'),
  count_on_page = Number($.find('#product-show option')[2].innerText),
  paginationNum,
  paginationCount = 0,
  page_num = 1,
  filter_value = [],
  ...
  getSavedFilters,
  selectedCount = Number($.find('#product-show option')[2].innerText),
  choosedType = 0,
  sizeValue = [];
...
let getTypes = () => {
	...
	$( '.type_name' ).each(function( index ) {
		type_names.push( $( this ).data('id') );
	});
	return type_names;
}

$('#filter-tip input').each(function () {
  if (getTypes().includes($(this).data('id'))) $( this ).prop('checked', true);
})

let flagFilter = false;
let getTypeFilters = () => {
  if ( getTypes().length > 0 && !flagFilter) {
    console.log('display:block 44');
    $('.CFBlock').css('display', 'block');
    flagFilter = true;
    $( '.type_name' ).each(function( index ) {
      $('.choosedFilter').append('' +
              '<li class="appedned__item">' +
              '<span class="item">' + $(this).data("name") + '</span>' +
              '<i class="fa fa-times-circle removeType__Item" data-name="' + $(this).data("name") + '" aria-hidden="true"></i>' +
              '</li>');
    });
    RemoveItem();
  }
...
function init() {
  $('.sidebar-container input[type=checkbox]').on('change', function(event) {
    ...
    var target = $(this)[0].parentNode.parentNode;
    targetID = $(this)[0].id;

    let type = 'sex';
    if (targetID.includes('season')) type = 'season';
    else if (targetID.includes('size')) type = 'size';
    else if (targetID.includes('manufacturer')) type = 'manufacturers';
    else if (targetID.includes('countries')) type = 'countries';
    else if (targetID.includes('category')) type = 'category';
    else if (targetID.includes('color')) type = 'color';
    ...
    if ($(this).is(':not(:checked)')) {
    	if( targetID.includes('type')) {
		    ...
		    $( '.type_name' ).each(function( index ) {
		      if ( name != $( this ).data('name')) type_names.push( $( this ).data('name') );
		    });
		    if( type_names.length > 1) window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }/types/${ type_names.join() }`;
		    else if( type_names.length == 1) window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }/${ type_names.join() }`;
		    else window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }`;
    	} else {
	      var unchecked = $(this)[0].defaultValue,
	      ...
	      $.ajax({
	        ...
	        data: {
	          ...
	          type_id: getTypes(),
	          brand_id: $('meta[name="brand_id"]').attr('content'),
	          nov_id: $('meta[name="nov_id"]').attr('content'),
	          page_num: 1,
    ...
    if ($(this).is(':checked')) {
      console.log('checked');
      if (!targetID.includes('type')) {
        values.push([targetID, $(this)[0].defaultValue, type]);
        localStorage.setItem('filterValues', JSON.stringify(values));
      } else {
        $('.product--block').append('<div class="preloader"><i></i></div>');
        console.log(`https://rostovka.net/${ $('.categoryPage')[0].dataset.link }/${ $(this).data('href') }`);
        if ( $(this).data('href').split(',').length > 1 ) window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }/types/${ $(this).data('href') }`;
        else window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }/${ $(this).data('href') }`;
      }
    }
    ...
    if (values.length !== 0) {
      ...
      for (var y = 0; y < values.length; y++) {
        ...
        if(values[y][1] == 'Детское') $('#detskoe-pol').css('display', 'block');
    ...
    if (msg.length > 0) {
      for (var i = 0; i < msg.length; i++) {
        ...
        data[i] = {
          ...
          prodNew: msg[i].prodNew,
          prodSale: msg[i].prodSale,
          specSale: msg[i].specsale,
...
function drawItems(pageList) {
  ...
  if(currentPage != paginationCount && paginationCount > 1)
    $('#target').append(`<li id="showmore" onclick="showMore()" class="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-xs-12 pull-left product-item showmore" style="pointer-events: auto; border: 1px solid rgba(0, 0, 0, 0.2) !important; cursor: pointer; height:350px;">
...
function RemoveItem() {
  $('.removeType__Item').on('click', function(e) {
    $('.product--block').append('<div class="preloader"><i></i></div>');
    let type_names = [];
    $( '.type_name' ).each(function( index ) {
      if (e.target.dataset.name != $( this ).data('name')) type_names.push( $( this ).data('name') );
    });
    if( type_names.length > 1) window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }/types/${ type_names.join() }`;
    else if( type_names.length == 1) window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }/${ type_names.join() }`;
    else window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }`;
  ...
  $('.removeAppended__Item').on('click', function() {
    ...
    if (sizeRemove !== null) {
      ...
      if ($('.choosedFilter li').length === 0) {
        $('.CFBlock').css('display', 'none');
        console.log('display:none 989');
        $('#detskoe-pol').css('display', 'none');
      }
      ...
      if ($('.choosedFilter li').length === 0) { $('.CFBlock').css('display', 'none'); console.log('display:none 1037');} 
...
$('.removeallFilters span').on('click', function() {
  ...
  if (values.length === 0) {
    $('.CFBlock').css('display', 'none');
    console.log('display:none 1089');
    $('#detskoe-pol').css('display', 'none');
  }

  $('input[type=checkbox]').prop('checked', false);

  return window.location.href = `https://rostovka.net/${ $('.categoryPage')[0].dataset.link }`
...
var GetSlideValue = function() {
  ...
  $.ajax({
    ...
    if (msg.length !== 0) {
      ...
      $('.product-filter-content').css('display', 'none');
      $('.pagination-wraper').css('display', 'none');
      if($('.error--message') == 0) $('.product-list-view').append('<div class="col-md-12 error--message">Выбранные фильтры не дали результатов</div>')
    }
  }).fail(function(msg) {
...
$("#ex2").bootstrapSlider().on('slideStop', GetSlideValue);
  ...
  if (localStorage.getItem('sizeValues') !== null) {
    ...
    if (sizeValue[0].sizeValues[0] !== 10 || sizeValue[0].sizeValues[1] !== 50) {
      $('.choosedFilter').append('' +
        ...
        '<span class="item" data-type="' + sizeValue[0].sizeValues[0] + '">' + "Цена: " + sizeValue[0].sizeValues[0] + "-" + sizeValue[0].sizeValues[1] + '</span>' +
        '<i class="fa fa-times-circle removeAppended__Item deleteSize_button" aria-hidden="true"></i>' +
        '</li>');
      RemoveItem();
    } else if (values.length === 0) {
      $('.CFBlock').css('display', 'none');
      console.log('display:none 1421');
      flagFilter = false;
      getTypeFilters();
    }
  }
...
$("#minchoose").on('change', function() {
  var value = $("#ex2").data('bootstrapSlider').getValue();
  var min = parseInt($(this).val());
  if(!min) min = value[0];
  $("#ex2").data('bootstrapSlider').setValue([min, value[1]]);  
  GetSlideValue();
  console.log(1297);
  updData();
});

$("#maxchoose").on('change', function() {
  var value = $("#ex2").data('bootstrapSlider').getValue();
  var max = parseInt($(this).val());
  if(!max) max = value[1];
  $("#ex2").data('bootstrapSlider').setValue([value[0], max]);  
  GetSlideValue();
});

$("#ex2").bootstrapSlider().on('change', function() {
  var newVal = $('#ex2').data('bootstrapSlider').getValue();

  $("#minchoose").val(newVal[0]);
  $("#maxchoose").val(newVal[1]);
});

function updData() {
  $.ajax({
    method: 'POST',
    url: $('meta[name="root-site"]').attr('content') + "/api/upd",
    data: {
      _token: $('meta[name="csrf-token"]').attr('content'),
      category_id: $('meta[name="category_id"]').attr('content'),
      type_id: getTypes(),
    ...
    if (msg.sizes.length == 0) $('#fsizes').append('<p>Нет подходящих размеров</p>');
    msg.sizes.map( size => {
      $('#fsizes').append(`<div class="col-sm-4" style="padding: 0 5px 5px 0"><input id="size${size.id}" type="checkbox" name="size${size.id}" value="${size.name}" data-value="size${size.id}" class="checkbox-input" style="display:none"><label for="size${size.id}" class="checkbox-label">${size.name}</label></div>`);  
      if (values.length !== 0)
      for (var y = 0; y < values.length; y++)
        if(values[y][1] == size.name)
          $(`#size${size.id}`).prop('checked', true);
    });
    ...
    $("#minchoose").val(newVal[0]);
    $("#maxchoose").val(newVal[1]);
    $('.sidebar-container input[type=checkbox]').off();
    init();
  })
}
    ...
    if ( page_num <= paginationCount ) {
      ...
      $.ajax({
        ...
        if (msg.length !== 0) {
          ...
          // $('.paginationItems a').removeClass('current');
          // $(`.paginationItems a:contains('${page_num}')`)[0].classList.add('current');
...
if($('.CFBlock').css('display') == 'none' && getTypes().length > 0) getTypeFilters()

if ($('#filter-category')[0] && $('#filter-category .checkbox:last-child')[0].innerText.indexOf('Детское') != -1) $('#detskoe-pol').css('display', 'block');
