                        ...
                        <div class="row">
                            ...
                            <div class="col-md-5">
                                    ...
                                    <div class="slider1">
                                        @foreach( $product -> photos -> groupBy('photo_url') as $url => $photo)
                                            @if ($url != 'none')
                                                ...
                                                <img class="modalImg" src="{{asset('images/products/' . $url)}}" style="max-height: 350px;left: 50%;transform: translateX(-50%);"/>
                                            @endif
                                        @endforeach
                                    </div>
                                </div>
                                ...
                                <div class="col-md-12" style="margin-top: 140px;">
                                    <p align="center" style="font-size: 16px;">Посмотреть всю продукцию этого бренда: <a href="{{url('/brand/'.$product -> manufacturer -> name)}}" style="text-decoration: underline; color: #0b599a;">{{ $product -> manufacturer -> name }}</a></p>
                                </div>
                            ...
                            <div class="col-md-5">
                                <div class="product-page-content">
                                    ...
                                    <div class="product-meta">
                                        ...
                                        <span>Пар в @if($product -> category -> id == 4)упаковке@elseростовке@endif: <span class="sku"
                                                                    itemprop="sku">{{$product -> rostovka_count}}</span></span>
                                        <span>Пар в @if($product -> category -> id == 4)паллете@elseящике@endif: <span class="category"
                                ...
                                <div class="single-variation-wrap">
                                    <div class="product-price">
                                        <span class="price {{$product -> specsale}}">{{$product -> prise}} грн</span>
                                    </div>
                                        ...
                                        <div class="radio lft choosed" data-set="boxset">
                                            <label>
                                                <input type="radio" name="optradio" value="0" style="width:25px; height:40px;"
                                                       checked onclick="getSelect(event)" data-id="box">в @if($product -> category -> id == 4)паллете@elseящике@endif
                                                <span class="boxPrice"><span
                                                            class="iPrice">{{$product -> prise * $product -> box_count}}</span> <sup>грн</sup> <span
                                                            class="forBag">за 1 @if($product -> category -> id == 4)паллету@elseящик@endif</span></span>
                                            </label>
                                        </div>
                                            ...
                                            <div class="radio rth disable" data-set="rotovkaset">
                                                <label>
                                                    ...
                                                    <span><span class="iPrice">{{$product -> prise * $product -> rostovka_count}}</span> <sup>грн</sup> <span
                                                                class="forBag">за 1 @if($product -> category -> id == 4)упаковку@elseростовку@endif</span></span>
                                                </label>
                                            </div>
                                        @endif
...
@section('scriptSlider')
    <script>
        jQuery(document).ready(function($) {
            ...
            $('#myModal').on('click', (e) => {
                if ( !$(e.target).is('img') ) modal.css('display', 'none');
            });
        });
    </script>
@endsection
