...
function createMessage($data, $sms, $captions) {
    ...
    foreach ($data as $item) {
        if ($item['key'] === 'receiver') {
            //...
        } else if ($item['text'] === 'email' && $from_user) {
            $from = $item['value'];
        } else if ($sms) {
            if ($item['sms'] == 'true') {
                if ($captions == 'true') {
...
if (!empty($_POST)) {
    ...
    $subject = 'UGG UK net - хотят купить товар';
    $data = $_POST['array'];
    $sms = $_POST['sms'];
    $message = '<p> клиент '.$_POST['array']['client'].'</p>'.
        '<p> Телефон: '.$_POST['array']['phone'].'</p>'.
        '<p> Товар: '.$_POST['array']['prodName'].'</p>'.
        '<p> ID товара: '.$_POST['array']['prodID'].'</p>'.
        '<p> Цена товара: '.$_POST['array']['prodPrice'].'</p>'.
        '<p> Размер: '.$_POST['array']['size'].'</p>';
    ...
    $message .= '<div style="background:#bfd4ac;border:1px solid #627650;padding:10px;margin:10px 0;">IP: <a href="http://iptool.pro/#'.$ip.'">'.$ip.'</a></div>';
    $headers = "Content-type: text/html; charset=utf-8\r\n";
    $headers .= "От: support@ugg-uk.net <".$from.">\r\n";
    $smtp = $_POST['smtp'];
